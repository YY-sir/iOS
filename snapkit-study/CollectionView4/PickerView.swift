...
//        self.itemSize = (self.collectionView?.frame.size)!
        self.itemSize = CGSize(width: 260, height: (self.collectionView?.frame.height)!)
...
//        CATransform3D trans3D = CATransform3DIdentity;
        ...
        let angleOffset: CGFloat = offset / (self.collectionView?.frame.height)!
        let angle: CGFloat = CGFloat((Float(indexPath.row) + Float(angleOffset)) / Float(itemCount) * Float(M_PI) * 2)
